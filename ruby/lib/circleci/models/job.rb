  ...
  class Job
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CircleCi::Job` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CircleCi::Job`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'approval_request_id')
        self.approval_request_id = attributes[:'approval_request_id']
      end
    end
    ...
    def valid?
      ...
      type_validator = EnumAttributeValidator.new('Object', ['build', 'approval'])
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('Object', ['build', 'approval'])
    ...
    def hash
      [canceled_by, dependencies, job_number, id, started_at, name, approved_by, project_slug, status, type, stopped_at, approval_request_id].hash
    end
