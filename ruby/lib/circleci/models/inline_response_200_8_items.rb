  ...
  class InlineResponse2008Items
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CircleCi::InlineResponse2008Items` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CircleCi::InlineResponse2008Items`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      status_validator = EnumAttributeValidator.new('Object', ['success', 'failed', 'error', 'canceled', 'unauthorized'])
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('Object', ['success', 'failed', 'error', 'canceled', 'unauthorized'])
    ...
    def hash
      [id, branch, duration, created_at, stopped_at, credits_used, status].hash
    end
