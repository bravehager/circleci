  ...
  class Workflow1
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CircleCi::Workflow1` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CircleCi::Workflow1`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      tag_validator = EnumAttributeValidator.new('Object', ['setup'])
      return false unless tag_validator.valid?(@tag)
      return false if @status.nil?
      status_validator = EnumAttributeValidator.new('Object', ['success', 'running', 'not_run', 'failed', 'error', 'failing', 'on_hold', 'canceled', 'unauthorized'])
    ...
    def tag=(tag)
      validator = EnumAttributeValidator.new('Object', ['setup'])
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('Object', ['success', 'running', 'not_run', 'failed', 'error', 'failing', 'on_hold', 'canceled', 'unauthorized'])
