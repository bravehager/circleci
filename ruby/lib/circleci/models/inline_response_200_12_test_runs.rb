...
module CircleCi
  class InlineResponse20012TestRuns
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CircleCi::InlineResponse20012TestRuns` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CircleCi::InlineResponse20012TestRuns`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'test_counts')
        self.test_counts = attributes[:'test_counts']
      end
    end
    ...
    def list_invalid_properties
      ...
      if @test_counts.nil?
        invalid_properties.push('invalid value for "test_counts", test_counts cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @test_counts.nil?
      true
    end
    ...
    def hash
      [pipeline_number, workflow_id, success_rate, test_counts].hash
    end
